...
import type { DsNativeImageAttrs, DsPictureBindingOptions, DsPictureComponentProps } from './types.js';
import { useDsPictureProps } from './hooks.js';

export const DsPicture = forwardRef<HTMLPictureElement, DsPictureComponentProps>(function DsPicture(
  {
    src,
    alt,
    width,
  ...
  const options = useMemo<DsPictureBindingOptions>(() => {
  ...
  const { pictureProps, sources, imgProps } = useDsPictureProps(options);
