...
const operationsGenerator = createOperationsGenerator({
  joinWith: '',
  formatter: (key: string, value: number | string | string[]) =>
    `-/${key}/${Array.isArray(value) ? value.join('/') : value}/`
});

const providerSetup = defineProvider<Partial<UploadcareOptions>>({
  getImage: (uuid, { modifiers, cdnURL = 'https://ucarecdn.com' }) => {
    // If width or height is specified, use resize instead
    if (modifiers?.width || modifiers?.height) {
      modifiers.resize = `${modifiers?.width || ''}x${modifiers?.height || ''}` as
        `x${number}` | `${number}x${number}` | `${number}x`;
...
export type UploadcareProviderOptions = Partial<ProviderOptionsOf<typeof providerSetup>>;

export function uploadcareProvider(options: UploadcareProviderOptions = {}) {
  return configureProvider(providerSetup, options, 'uploadcare');
}
