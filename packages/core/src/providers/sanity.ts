...
const providerSetup = defineProvider<SanityOptions>({
  getImage: (src, { modifiers, projectId, dataset, baseURL = sanityCDN }) => {
    ...
    if (modifiers.crop && typeof modifiers.crop !== 'string' && sourceWidth && sourceHeight) {
      ...
      modifiers.rect = [left, top, right - left, bottom - top]
        .map((i) => i.toFixed(0))
...
export type SanityProviderOptions = Partial<ProviderOptionsOf<typeof providerSetup>>;

export function sanityProvider(options: SanityProviderOptions = {}) {
  return configureProvider(providerSetup, options, 'sanity');
}
