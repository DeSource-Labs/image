...
const generateColorKeys = () => {
  ...
  return Object.fromEntries(keysNeedingHashDeletion.map((key) => [key, (value: string) => deleteHash(value)]));
};
...
const providerSetup = defineProvider<SirvOptions>({
...
export type SirvProviderOptions = Partial<ProviderOptionsOf<typeof providerSetup>>;

export function sirvProvider(options: SirvProviderOptions = {}) {
  return configureProvider(providerSetup, options, 'sirv');
}
