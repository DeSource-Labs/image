<script lang="ts">
  ...
  import { getDsPictureProps, preloadImage, splitDsPictureAttributes, toDsImageInput } from './bindings.js';
  import { getDsImageConfig } from './context.js';
  import type { DsPictureBindingOptions, DsPictureComponentProps } from './types.js';
  ...
  let {
    ...
    onerror,
    ...rest
  }: DsPictureComponentProps = $props();

  const config = getDsImageConfig();
  let loaded = $state(false);
  const distributed = $derived(splitDsPictureAttributes(rest));
  const bindingOptions = $derived<DsPictureBindingOptions>({
    src,
    alt,
    width,
    height,
  ...
  const elementProps = $derived(getDsPictureProps(bindingOptions, loaded));
