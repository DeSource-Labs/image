<script lang="ts">
  ...
  import { DsImage, DsPicture, createDsImageBindings, setDsImageConfig } from '@desource/image-svelte';

  const config = setDsImageConfig({
    provider: 'ipx',
...
<main>
  ...
  <section class="grid">
    ...
    <article>
      <span>dsImageAction</span>
      <img
        data-testid="image-action"
        class="media"
        alt="Aurora reflected in a lake"
        use:dsImageAction={actionOptions}
      />
    </article>

    <article>
      <span>dsImageAttachment (Svelte 5.29+)</span>
      <img
    ...
    <article>
      <span>dsPictureAttachment (Svelte 5.29+)</span>
      <picture
